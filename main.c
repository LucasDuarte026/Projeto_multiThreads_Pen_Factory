...
void comprador(void)
{
    while (1) //   Variável controlada pelo criador para terminar esta thread
    {
        ...
        pthread_mutex_unlock(&canetas_do_comprador);				// Revoga acesso a quantidade de canetas a serem compradas
        pthread_mutex_unlock(&acessar_deposito_caneta);				// Revoga acesso a quantidade de canetas no depósito
        pthread_cond_signal(&sinal_ao_criador);						// Sinaliza ao criador que a compra foi feita
